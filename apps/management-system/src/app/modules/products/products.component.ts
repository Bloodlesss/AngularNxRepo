...
} from '../orders/shared/model/order.model';
...
export class ProductsComponent implements OnInit, OnDestroy {
  ...
  organizations$: Observable<Organization[]> =
    this.organizationFacade.selectOrganizations$;
  orders$: Observable<Order[]> = this.ordersFacade.selectOrders$;
  pendingOrganization$: Observable<boolean> =
    this.organizationFacade.selectOrganizationsPending$;
  pendingOrder$: Observable<boolean> = this.ordersFacade.selectOrdersPending$;
  pendingProduct$: Observable<boolean> =
    this.productFacade.selectProductsPending$;
  ...
  initializeSubscriptions(): void {
    ...
    this.subscriptions.add(
      this.orders$
        .pipe(filter((orders: Order[]) => orders.length > 0))
    ...
    this.subscriptions.add(
      this.organizations$
        .pipe(filter((organization: Organization[]) => organization.length > 0))
    ...
    this.pendingState = combineLatest([
      this.pendingOrder$,
      this.pendingOrganization$,
      this.pendingProduct$,
    ]).pipe(
      map(([a, b, c]: boolean[]) => a || b || c),
      distinctUntilChanged()
    );
  }
  initializeForm(): void {
  ...
  checkDeleteDependencyValidity(id: number): boolean {
    const organizationState: boolean = this.organizationsData.some(
      (organization: Organization) => organization.products.includes(id)
    );
    const orderState: boolean = this.ordersData.some((order: Order) =>
      order.products.some((product: OrderProduct) => product.id === id)
    );
