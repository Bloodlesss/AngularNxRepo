import {
  CREATE_PRODUCT_MOCK,
  PRODUCT_MOCK,
} from 'src/specs/mocks/product.mock';
...
describe('authReducer', (): void => {
  ...
  const requestSendingActions: any[] = [
    getProducts(),
    updateProduct(PRODUCT_MOCK),
    deleteProduct({ id: 1 }),
    postProduct(CREATE_PRODUCT_MOCK),
  ];
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const singleTestForGenericAction: (
    description: string,
    action: any,
    outputState: ProductsState
  ) => void =
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (description: string, action: any, outputState: ProductsState): void => {
      it(`${description} for ${action.type} action`, (): void => {
        const state: ProductsState = productsReducer(pendingState, action);
  ...
  describe('request sending actions', (): void => {
    requestSendingActions.forEach(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (action: any): void => {
        singleTestForGenericAction(
          'should add pending flag',
          action,
          pendingState
        );
      }
