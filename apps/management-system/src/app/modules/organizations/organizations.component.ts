...
export class OrganizationsComponent implements OnInit, OnDestroy {
  ...
  initializeSubscriptions(): void {
    ...
    this.subscriptions.add(
      this.organizations$.subscribe((organization?: Organization[]) => {
  ...
  initializeForm(): void {
    this.organizationFormGroup = this.formBuilder.group({
      ...
      [this.organizationFormEnum.OrganizationType]: [
        OrganizationType.Buyer,
        [Validators.required],
      ],
    });
  ...
  onRowEditSave(): void {
    ...
    this.organizationFacade.updateOrganization(
      this.organizationEditFormGroup.value
    );
  ...
  checkDeleteDependencyValidity(id: number): boolean {
    const orderState: boolean = this.ordersData.some(
      (order: Order) => order.organization === id
    );
    if (orderState) {
